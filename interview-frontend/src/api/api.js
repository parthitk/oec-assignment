...
export const assignUsersToProcedures = async (planId,procedureId,userId,operation) => {
    const url = `${api_url}/Plan/AddUserToProcedure`;
   var command = { planId: planId, procedureId: procedureId,userId:userId,operation:operation };
    const response = await fetch(url, {
        method: "POST",
        headers: {
            Accept: "application/json",
...
export const getUserProcedure = async () => {
    const url = `${api_url}/UserProcedure`;
    var response = await fetch(url,{
        method:'GET'
    });
    if(!response.ok) throw new Error("Failed to get usersprocedure");
    return await response.json();
}